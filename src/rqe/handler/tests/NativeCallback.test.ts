...
it("declaredFunctionToHandler should handle the implicit 'task' param", async () => {
    ...
    const task = new Task({withQuery:query,output: new Stream(),queryParameters} as any);
    runTaskCallback(task, handler.run);

    task.output.collectEventsSync();
    //await task.output.wait();

    //console.log(task.output.collectEventsSync())
